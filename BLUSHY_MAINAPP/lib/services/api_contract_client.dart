...
enum ApiState {
  ...
  /// Somebody decided this may not be shown. On the partner side that is her
  /// permission switch; on her own side it is a stage that does not do cycle
  /// tracking. Either way a person chose it.
  restricted,

  /// There is no valid session, so the server would not say either way.
  ///
  /// Kept apart from [restricted] because the two were the same value and the
  /// wording that survived was the partner's: a signed-out woman was told
  /// "This information has not been shared with you" about her own cycle. A
  /// 401 is not a decision anyone made about her -- it is a login that ran
  /// out.
  unauthenticated,
...
class ApiResult<T> {
  ...
  bool get isUnauthenticated => state == ApiState.unauthenticated;
