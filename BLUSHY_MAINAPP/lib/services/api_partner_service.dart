...
class ApiPartnerService {
  ...
  Future<Map<String, dynamic>> acceptInviteLink(String inviteCode) async {
    try {
      ...
      // The status separates a verdict from a failure to ask. A refused code
      // is spent; a cold start that timed out is not, and the caller has to be
      // able to keep the code and try again.
      return {'error': message, 'statusCode': e.response?.statusCode};
    } catch (e) {
      return {'error': e.toString()};
    }
  }
