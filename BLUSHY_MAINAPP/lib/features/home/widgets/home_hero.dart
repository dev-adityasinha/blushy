...
class CycleRingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlushySurface(
      ...
      // "Ready" with no day in it is not ready.
      //
      // `_Ready` falls back to `day ?? 1` and `length ?? 28`, and renders a
      // null phase as "Menstrual Phase" -- so an account that has logged
      // nothing was shown "Day 1 / Menstrual Phase / Next cycle begins in 27
      // Days", in the same confident type as a real reading. Reported as a
      // cycle jumping from day 20 to day 1 on its own.
      //
      // `insufficient_data` maps to ready on purpose, because a cycle with
      // too little history for predictions still has a real day to show. The
      // test is the day itself, not the state.
      child: switch (state) {
        CycleCardState.loading => const _RingSkeleton(),
        CycleCardState.noTracking => _NoTracking(onSetUp: onSetUp),
        CycleCardState.ready =>
          cycleDay == null ? _NoTracking(onSetUp: onSetUp) : _Ready(card: this),
      },
    );
  }
}
