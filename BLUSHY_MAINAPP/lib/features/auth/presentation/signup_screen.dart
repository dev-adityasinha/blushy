...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _showOtpVerificationDialog(String email) async {
    // The boxes start empty.
    //
    // They used to be pre-filled from the code the server returned in the
    // signup response, which makes the check ceremonial: the point of a code
    // sent to an address is that only someone reading that inbox has it.
    // Typing it in for them proves nothing, and on a server that returns the
    // code it would let anyone register an address they do not control.
